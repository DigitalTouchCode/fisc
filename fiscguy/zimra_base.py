...
class ZIMRA:
    ...
    def __init__(self):
        ...
        self.device_id = int(os.getenv("DEVICE_ID", "23265"))
    ...
    def register_device(self):
        ...
        try:
            response = requests.post(
                f"{self.registration_url}/RegisterDevice", 
                json=payload, 
                headers=headers
            )
    ...
    def submit_receipt(self, receipt_data, credit_note_data, hash_value, signature):
        ...
        Args:
            ...
            credit_note_data (dict): Credit note data with 'receipt' key
        ...
        try:
            ...
            request = requests.post(
                ...
                json=receipt_data or credit_note_data, 
                headers=headers, 
                cert=(self.certificate_path, self.certificate_key)
            )
    ...
    def submit_credit_note(self, credit_note_data, hash_value, signature):
        """
        Submit a credit note to ZIMRA.
        
        Args:
            credit_note_data (dict): Credit note data
            hash_value (str): Hash of the credit note
            signature (str): Signature of the credit note
        ...
        try:
            ...
            logger.info(f"Submitting credit note: {credit_note_data}")
            
        except Exception as e:
            logger.error(f"Error preparing credit note: {e}")
            return {"error": str(e)}
        ...
        try:
            ...
            logger.info(f"Credit note response: {response.json()}")
            response.raise_for_status()
            logger.info("Credit note submitted successfully.")
            return response.json()
        except Exception as e:
            logger.error(f"Error submitting credit note: {e}")
            return {"error": str(e)}

    def open_day_api(self, payload):
        """
    ...
    def ping(self):
        ...
        try:    
            ...
            logger.info(f"FDMS online. Next ping in {reporting_frequency * 60} seconds.")
